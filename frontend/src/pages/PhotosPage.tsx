...
export default function PhotosPage() {
  ...
  const fetchPhotos = async () => {
    try {
      const res = await photoApi.getAll(user.id);
      setPhotos(res.data);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const handleUpload = async () => {
    ...
    setUploading(true);
    try {
      await photoApi.upload(user.id, selectedFile, caption || undefined);
      ...
      <div style={{
        ...
        <div
          ...
          style={{
            ...
            textAlign: 'center',
            cursor: 'pointer',
            background: preview ? 'rgba(200,105,74,0.04)' : 'var(--cream)',
            transition: 'all 0.2s',
            marginBottom: '20px',
          }}
        >
          {preview ? (
            <div>
              ...
              />
              <p style={{ color: 'var(--accent)', marginTop: '12px', fontSize: '0.9rem' }}>
                クリックして別の写真を選ぶ
              </p>
            </div>
          ) : (
            <>
              ...
              <p style={{ color: 'var(--brown)', fontSize: '1rem', marginBottom: '8px' }}>
                クリックして写真を選ぶ
              </p>
      ...
      {lightbox && (
        ...
        >
          <div onClick={e => e.stopPropagation()} style={{ textAlign: 'center' }}>
            <img
              src={`${API_BASE}${lightbox.photo_url}`}
            ...
            />
            {lightbox.caption && (
              <p style={{ color: 'white', marginTop: '16px', fontSize: '1rem' }}>
                {lightbox.caption}
              </p>
            )}
            <button

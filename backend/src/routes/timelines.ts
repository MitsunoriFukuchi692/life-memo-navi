...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const result = await pool.query(
      ...
      [user_id, year, month || null, event_title, event_description || null, photo_id || null, field_type]
    );
...
router.put('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const result = await pool.query(
      ...
      [event_title || null, event_description || null, month || null, year || null, photo_id || null, id]
    );
    if (result.rows.length === 0) return res.status(404).json({ error: 'Timeline not found' });
    res.json(result.rows[0]);
  } catch (error: any) {

...
// カテゴリ設定
// ========================================
const SHUKATSU_CATEGORIES: Record<string, {
  label: string;
  systemPrompt: string;
...
router.post('/save', async (req: Request, res: Response) => {
  try {
    ...
    // 既存データを削除してから保存（上書き）
    await pool.query(
      'DELETE FROM shukatsu_notes WHERE user_id = $1 AND category = $2',
      [userId, category]
    );

    for (let i = 0; i < qa_pairs.length; i++) {
      const { question, answer } = qa_pairs[i];
      if (!answer) continue; // 回答がない質問はスキップ
      await pool.query(
        `INSERT INTO shukatsu_notes (user_id, category, question, answer, display_order)
         VALUES ($1, $2, $3, $4, $5)`,
        [userId, category, question, answer, i]
      );
    }
...
router.get('/notes/:userId', async (req: Request, res: Response) => {
  try {
    ...
    // カテゴリ別に整理
    const notesByCategory: Record<string, { question: string; answer: string }[]> = {};

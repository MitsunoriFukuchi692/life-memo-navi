...
// ============================================================
const getEraHintEN = (questionId: number, birthYear: number): string => {
  const age10 = birthYear + 10;
  const age20 = birthYear + 20;
  const age35 = birthYear + 35;

  const getEventsEN = (year: number): string => {
    if (year < 1940) return 'the prewar era — economic hardship and rising global tensions';
    if (year < 1950) return 'the World War II years and the immediate postwar period';
    if (year < 1960) return 'the 1950s — postwar recovery, the dawn of television, and the baby boom';
    if (year < 1970) return 'the 1960s — the space race, civil rights movements, rock and roll, and rapid social change';
    if (year < 1980) return 'the 1970s — oil crises, the end of the Vietnam War, and shifting cultural values';
    if (year < 1990) return 'the 1980s — the personal computer revolution, pop culture, and the Cold War winding down';
    if (year < 2000) return 'the 1990s — the early internet, globalization, and rapid change';
    if (year < 2010) return 'the 2000s — smartphones, social media, and global uncertainty';
...
const getCompanyEraHintEN = (questionId: number, foundingYear: number): string => {
  const plus10 = foundingYear + 10;
  const plus20 = foundingYear + 20;

  const getEventsEN = (year: number): string => {
    if (year < 1940) return 'the prewar era — controlled economies and wartime mobilization';
    if (year < 1950) return 'the postwar recovery — reconstruction, pent-up demand, and new opportunities';
    if (year < 1960) return 'the 1950s boom — rising consumer demand, industrial growth, and suburbanization';
    if (year < 1970) return 'the 1960s — rapid economic expansion, labor shortages, and social upheaval';
    if (year < 1980) return 'the 1970s — oil shocks, stagflation, and the rise of small business';
    if (year < 1990) return 'the 1980s — deregulation, globalization, and the personal computer era';
    if (year < 2000) return 'the 1990s — the dot-com boom, internet adoption, and global competition';
    if (year < 2010) return 'the 2000s — digital disruption, the financial crisis, and e-commerce growth';

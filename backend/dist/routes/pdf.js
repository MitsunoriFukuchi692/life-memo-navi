...
router.get('/generate/:user_id', async (req, res) => {
    try {
        ...
        const interviewResultRaw = await pool.query('SELECT * FROM interviews WHERE user_id = $1 AND field_type = $2 ORDER BY question_id', [user_id, field_type]);
        const interviewResult = {
            rows: interviewResultRaw.rows.map((row) => ({
                ...row,
                answer_text: row.answer_text ? decrypt(row.answer_text) : row.answer_text,
            })),
        };
        const timelineResultRaw = await pool.query('SELECT * FROM timelines WHERE user_id = $1 AND field_type = $2 ORDER BY year, month', [user_id, field_type]);
        const timelineResult = {
            rows: timelineResultRaw.rows.map((row) => ({
                ...row,
        ...
        const photoResult = await pool.query('SELECT * FROM photos WHERE user_id = $1 AND field_type = $2 ORDER BY uploaded_at', [user_id, field_type]);
        // 終活ノート取得（field_typeに関係なく常に取得）
        const shukatsuResult = await pool.query(`SELECT category, question, answer, display_order
       FROM shukatsu_notes
       WHERE user_id = $1
       ORDER BY category, display_order`, [user_id]);
        const shukatsuCategories = {

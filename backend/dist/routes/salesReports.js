...
// 暗号化して保存するフィールド
const ENCRYPTED_FIELDS = ['content', 'next_action', 'impression'];
const encryptReport = (data) => ({
    ...data,
...
router.get('/user/:user_id', async (req, res) => {
    try {
        ...
        const result = await pool.query(`SELECT * FROM sales_reports WHERE user_id = $1 ORDER BY report_date DESC, created_at DESC`, [user_id]);
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const result = await pool.query('SELECT * FROM sales_reports WHERE id = $1 AND user_id = $2', [id, req.user.id]);
...
router.post('/', async (req, res) => {
    try {
        ...
        const { report_date, visit_company, contact_person, purpose, content, next_action, impression, } = req.body;
        ...
        const result = await pool.query(`INSERT INTO sales_reports
        (user_id, report_date, visit_company, contact_person, purpose, content, next_action, impression)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       RETURNING *`, [user_id, report_date, visit_company || '', contact_person || '',
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const { report_date, visit_company, contact_person, purpose, content, next_action, impression, } = req.body;
        const enc = encryptReport({ content, next_action, impression });
        const result = await pool.query(`UPDATE sales_reports
       SET report_date = $1, visit_company = $2, contact_person = $3,
           purpose = $4, content = $5, next_action = $6, impression = $7,
           updated_at = NOW()
       WHERE id = $8 AND user_id = $9
       RETURNING *`, [report_date, visit_company || '', contact_person || '',
            purpose || '', enc.content || '', enc.next_action || '', enc.impression || '',
            id, req.user.id]);
        if (result.rows.length === 0)
            return res.status(404).json({ error: 'Not found' });
...
router.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const result = await pool.query('DELETE FROM sales_reports WHERE id = $1 AND user_id = $2 RETURNING id', [id, req.user.id]);
        if (result.rows.length === 0)
            return res.status(404).json({ error: 'Not found' });

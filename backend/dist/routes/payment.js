...
// メール送信ヘルパー関数
// ----------------------------------------
async function sendTrialWillEndEmail(toEmail, userName, planType) {
...
router.post('/cancel', async (req, res) => {
    try {
        ...
        const result = await pool.query('SELECT stripe_subscription_id FROM subscriptions WHERE user_id = $1', [userId]);
        if (result.rows.length === 0) {
        ...
        await pool.query(`UPDATE subscriptions SET status = 'cancel_at_period_end', updated_at = NOW()

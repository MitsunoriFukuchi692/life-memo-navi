...
router.put('/:id', async (req, res) => {
    try {
        ...
        const encryptedDescription = event_description ? encrypt(event_description) : null;
        const result = await pool.query('UPDATE timelines SET event_title = COALESCE($1, event_title), event_description = COALESCE($2, event_description), month = COALESCE($3, month), year = COALESCE($4, year), photo_id = COALESCE($5, photo_id), updated_at = NOW() WHERE id = $6 RETURNING *', [encryptedTitle, encryptedDescription, month || null, year || null, photo_id || null, id]);
        if (result.rows.length === 0)
            return res.status(404).json({ error: 'Timeline not found' });
        // 復号して返す
        res.json(decryptRow(result.rows[0]));

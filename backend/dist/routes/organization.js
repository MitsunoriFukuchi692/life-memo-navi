...
router.post('/import-members', async (req, res) => {
    ...
    try {
        ...
        const orgResult = await pool.query(`SELECT * FROM organizations WHERE org_code = $1 AND admin_email = $2`, [orgCode.toUpperCase(), adminEmail.trim()]);
        if (orgResult.rows.length === 0) {
        ...
        for (const member of members) {
            ...
            try {
                ...
                if (existingUser.rows.length > 0) {
                    ...
                    const alreadyMember = await pool.query('SELECT id FROM organization_members WHERE user_id = $1 AND organization_id = $2', [userId, org.id]);
                    if (alreadyMember.rows.length > 0) {
                        skippedCount++;
                        continue; // 既に会員 → スキップ
                    }
                    // 既存ユーザーを組織に追加
                    await pool.query(`INSERT INTO organization_members (organization_id, user_id, user_name, user_email, joined_at)
             VALUES ($1, $2, $3, $4, NOW())`, [org.id, userId, name.trim(), email.trim().toLowerCase()]);
                    ...
                    const newUser = await pool.query(`INSERT INTO users (name, age, email, password_hash, project_type, plan, email_verified)
             VALUES ($1, $2, $3, $4, $5, $6, true)
             RETURNING id`, [name.trim(), 0, email.trim().toLowerCase(), hashedPassword, 'jibunshi', 'standard']);
...
// 招待メール送信関数
// ========================================
async function sendWelcomeEmail(email, name, orgName, tempPassword) {
